...
extern uint8_t fouund_ecg;
	#include "u8g2.h"
	#include "mui.h"
	#include "mui_u8g2.h"
	extern u8g2_t u8g2;
	 extern mui_t ui;
extern void uint8_to_str(uint8_t num, char* str);
...
class Dali {
	...
	void     set_levelS(uint8_t level, uint8_t adr=0xFF); //set arc level
	void set_level(uint8_t level, uint16_t selectedAddress);
	
		void set_level_DAPC(uint8_t level, uint16_t selectedAddress);

	void set_OFF(uint16_t  selectedAddress) ;
	void set_ON(uint16_t  selectedAddress) ;

	void	goto_scene(uint16_t selectedAddress, uint8_t index);
	void	setTC(uint16_t selectedAddress, uint16_t tc);
	void setRGB(uint16_t  selectedAddress,union ColorRGBs ColorRGB) ;
	...
	uint8_t  commission(uint8_t init_arg=0xff);
	 	uint8_t  DPS_commission(uint8_t init_arg=0xff);
	...
	uint32_t find_addr();
		void SetNewAddress(uint8_t gearOldAddress,uint8_t  newAddress );
void swap_address(uint8_t gearOldAddress, uint8_t gearNewAddress, uint8_t newAddress );
	
	void ReadRandomGearAddress(uint8_t adr);
	  void CheckRandomGearOldAddress(uint8_t gearOldAddrRem,uint8_t gearNewAddrRem);
	   uint8_t easy_replace() ;
	
	uint8_t   query_actual_level(uint8_t adr);
	
	  uint8_t add_to_group(uint8_t value, uint8_t adr) ;

		    uint8_t remove_from_group(uint8_t value, uint8_t adr) ;

	
	uint8_t GetFirstFreeShortAddress();
	private:
	//-------------------------------------------------
	...
	uint8_t rxdata[DALI_RX_BUF_SIZE];     //received samples
